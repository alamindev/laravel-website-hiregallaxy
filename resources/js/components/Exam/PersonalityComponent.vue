<template>
  <div>
    <div
      v-if="loading"
      class="hiregallaxy_preloader d-flex justify-content-center mt-5"
    >
      <img
    ...
    <div v-if="exam_status" class="d-flex justify-content-center mt-5">
      <div
        class="hiregallaxy__start_exam py-5 d-flex flex-column align-items-center"
      >
    ...
    <div
      v-if="!isExamStart && !loading && !exam_status && !error"
      class="d-flex justify-content-center py-5 mt-5"
    >
      <div class="hiregallaxy__start_exam">
        <button class="btn btn-primary" @click="StartExam">Start test</button>
      </div>
    </div>
    <div
      v-if="isExamStart"
      class="hiregallaxy_main__section d-flex flex-column justify-content-center w-100"
    >
      <div
        class="hiregallaxy_main__header d-flex justify-content-between align-items-center"
      >
        <div
          class="hiregallaxy_time__section d-flex justify-content-between align-items-center"
        >
          <div class="hiregallaxy_time__left d-flex align-items-center">
            <i class="fa fa-clock-o"></i>
            <span class="font-weight-bold mx-2">TIME LEFT</span>
          </div>
          <div class="hiregallaxy__time d-flex">
            00:<timer :time="prettyTime"></timer>
          </div>
        </div>
      </div>
      <div
        class="hiregallaxy_main__question d-flex justify-content-start align-items-center"
        v-if="!showResult"
      >
        <div
          class="hiregallaxy_pagination__left_side d-flex align-items-center"
        >
          <i class="fa fa-file-text-o"></i>
          <h5 class="mb-0 mx-3">
            Total Questions:
            <span class="text-danger">{{ quiz.questions.length }}</span>
          </h5>
        </div>
          ...
          <div class="progressContainer">
            <div class="progress hiregallaxy__progress">
              <div
                ...
                :style="
                  'width:' +
                  Math.floor((progressIndex / quiz.questions.length) * 100) +
                  '%'
                "
              ></div>
            </div>
        ...
        <div class="row">
          <div class="col-lg-6">
            <div class="hiregallaxy_left__side text-left">
              <div
                class="hiregallaxy_question__no d-flex align-items-center justify-content-start"
              >
              ...
              <h5 class="hiregallaxy_question__alert m-0 py-4">
                Carefully read the question and answer accordingly.
              </h5>
              <p class="hiregallaxy__question m-0 font-weight-bold">
                {{ questionIndex + 1 }}.
                <span
                  class="question__image"
                  v-html="quiz.questions[questionIndex].question"
                ></span>
              </p>
            </div>
          </div>
          <div class="col-lg-6">
            <div class="hiregallaxy_right__side mt-5 mt-lg-0">
              <h5 class="hiregallaxy_choose mb-3">Choose Best Option</h5>
              <div class="hiregallaxy__options">
                <span
                  class="d-none question_id"
                  :data-questionid="quiz.questions[questionIndex].id"
                ></span>
                <div class="form-check">
                  ...
                  <label class="form-check-label" for="question_1">
                    {{ quiz.questions[questionIndex].answer_1 }}
                  </label>
                </div>
                <div class="form-check">
                  <input
                  ...
                  <label
                    ...
                    for="question_2"
                    v-html="quiz.questions[questionIndex].answer_2"
                  >
                  </label>
                </div>
              </div>
      ...
      <div
        v-if="!showResult"
        class="hiregallaxy_main__footer d-flex justify-content-lg-end justify-content-center"
      >
        <div class="hiregallaxy__next ml-3">
          <button
            :disabled="question == ''"
            v-if="!queIndex"
            class="btn btn-success hiregallaxy_custom_btn d-flex justify-content-between align-items-center"
            v-on:click="next()"
          >
            Next Question
          ...
          <button
            ...
            :disabled="question == ''"
            @click="fineshedExam"
            class="text-center btn btn-success hiregallaxy_custom_btn d-flex justify-content-between align-items-center"
          >
            Finished
          </button>
        </div>
      ...
      <div
        class="hiregallaxy__submit_result d-flex justify-content-center my-5 py-5"
        v-if="showResult"
      >
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      Axios.get("/candidates/personality/questions/" + this.id).then(
        async (res) => {
    ...
    storeAnswer() {
      const data = {
        answer_1: this.question == 0 ? "1" : "0",
        answer_2: this.question == 1 ? "1" : "0",
      };
    ...
    user_finished_exam() {
      Axios.get("/candidates/personality/check-status").then((res) => {
